...
class MenuManagerComponent extends UIComponent {
  ...
  // Override abstract method render()
  async render() {
    const items = await MenuStore.byCategory(this.#currentFilter);
  ...
  async openEditModal(id) {
    const allItems = await MenuStore.getAll();
...
class HistoryComponent extends UIComponent {
  ...
  async render() {
    const allOrders = await Orders.getAll();
    const done = allOrders.filter(o => o.status === 'selesai');
